...
project-explorer egeria-reports egeria --full    # also fetch annotation detail from Egeria
```
...
project-explorer survey arrow

# 3. Optionally publish to Egeria
project-explorer survey arrow --publish
```
...
project-explorer refresh arrow
...
project-explorer survey house_prices_global --data-path ~/datasets/house-prices
```
...
project-explorer add https://github.com/odpi/egeria \
...
project-explorer survey egeria-adapters egeria-frameworks

# Publish both (creates separate SurveyReports linked to the shared asset)
project-explorer survey egeria-adapters egeria-frameworks --publish
```
...
```
project-explorer add https://github.com/datasciencedojo/datasets
project-explorer survey datasets
```
...
$ project-explorer egeria-reports egeria --full
...
project-explorer survey egeria beeai_framework docling_git ml_llm_ops
...
project-explorer refresh --all --no-stats

# Re-index top-level only, then survey top-level only
project-explorer refresh --all --top-level
project-explorer survey --all --top-level --publish
```
...
```bash
project-explorer add https://github.com/odpi/egeria --name egeria-full
```
...
project-explorer survey egeria-adapters

# Survey several sub-projects together (one Egeria connection for --publish)
project-explorer survey egeria-adapters egeria-frameworks egeria-views --publish
...
project-explorer survey <slug> --publish
